...
def get_model(pretrained=True):
    ...
    # We will change the stride = 1 for the first layer since CIFAR-10, which has 32x32 images. This will retain resolution in the early layers. cast() exists to please my editor's pyright syntax check. 

    fblock = cast(nn.Conv2d , cast(nn.Sequential, model.features[0])[0])  # First Block

    # Modify the first layer to retain everything else and with stride = 1
